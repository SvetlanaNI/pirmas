   ...
   /* public class DropDownPage : BasePage
    {
		...
		public DropDownPage CheckListedStates(List<string> selectedElements)
        {
            ...
            {
				Assert.True(result.Contains(selectedElement), $"Should be {selectedElement}, but was {result}");
            }
			return this;
        } 
        ...
        {
			string result = ResultTextAllSelectedElement.Text;
			Assert.True(result.Contains(selectedElement), $"{selectedElement} is missing. {result}");
			return this;
        }
